...
if __name__ == "__main__":  
    ...
    parser = argparse.ArgumentParser(description="A script that takes a ros2bag file, and a topic name and outputs a csv file with the data.")

    # Add a positional argument
    parser.add_argument('bag_file', type=str, help='A full path to your .db3 file',)
    parser.add_argument('topic_name', type=str, help='A topic name to export to csv')
    parser.add_argument('csv_file_path', type=str, nargs='?', default=f'output_{time.time()}.csv', help='A full path to your output .csv file')

    # Parse the arguments
    args = parser.parse_args()

    # If the argument is empty, show a usage message and exit
    if not args.bag_file or not args.topic_name:
        print("Usage: script.py --bag-file <> --topic-name <> --csv-file-path <>")
        sys.exit(1)

    ### connect to the database
    conn, c = connect(args.bag_file)
    ...
    t, msgs = getAllMessagesInTopic(c, args.topic_name, print_out=True)

    # Deserialize the message
    msg_type = get_message(type_map[args.topic_name])  # Assuming type_map is a dictionary mapping topic names to message types
    
    # Open the CSV file for writing
    with open(args.csv_file_path, 'w', newline='') as csvfile:
        # Create a CSV writer
